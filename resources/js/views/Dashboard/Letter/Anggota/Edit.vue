      ...
      <div class="card">
        <div class="card-header pb-0">
          <div class="d-flex justify-content-between align-items-center">
            <p class="mb-0">Edit Anggota</p>
            <button v-on:click="back" class="badge border-0 bg-gradient-danger">back</button>
          </div>
        </div>
        <div class="card-body">
          <form v-on:submit.prevent="formOnSubmit">
            <div class="form-group">
              ...
              <input id="nama" v-model="anggota.name" type="text" autofocus class="form-control"
                :class="{ 'is-invalid': errors.name }" name="nama" placeholder="Masukan Nama" />
              <small v-if="errors.name" class="text-danger">{{ errors.name }}</small>
            </div>
            <div class="form-group">
              <label for="nim" class="form-control-label">NIM</label>
              <input id="nim" type="number" class="form-control" :class="{ 'is-invalid': errors.nim }" name="nim"
            ...
            <div class="form-group">
              ...
              <input id="no_hp" type="tel" class="form-control" :class="{ 'is-invalid': errors.no_hp }" name="no_hp"
                v-model="anggota.no_hp" placeholder="Masukan Nomor HP" />
              <small v-if="errors.no_hp" class="text-danger">{{ errors.no_hp }}</small>
            </div>
            <div class="form-group">
              <label for="email" class="form-control-label">Email</label>
              <input id="email" type="email" class="form-control" :class="{ 'is-invalid': errors.email }" name="email"
                v-model="anggota.email" placeholder="Masukan Email" />
              <small v-if="errors.email" class="text-danger">{{ errors.email }}</small>
            </div>
            <div class="form-group">
              <label for="angkatan" class="form-control-label">Angkatan</label>
              <select id="angkatan" name="id_angkatan" class="form-select" v-model="anggota.id_angkatan"
            ...
            <div class="d-flex flex-column flex-sm-row">
              <button type="button" v-on:click="backValue" class="btn btn-success btn-sm mb-0">
                Ulang
              </button>
              <button class="btn btn-primary btn-sm ms-sm-auto my-1 my-sm-0">
                Submit
              </button>
              <button type="reset" v-on:click="resetButton" class="btn btn-secondary btn-sm mb-0 ms-sm-1">
                Reset
              </button>
            </div>
          </form>
...
export default {
  ...
  watch: {
    anggotas: {
      ...
      handler(items) {
        ...
        this.id_anggota = this.$route.params.id;
        const anggota = items.find(k => k.id == this.id_anggota);
        if (anggota) {
  ...
  methods: {
    async formOnSubmit() {
      ...
      try {
        const user = await axios.put("anggota/"+this.id_anggota,
          {
    ...
    // async getAnggotaById() {
    //   try {
    //     const anggota = await axios.get("anggota/"+this.id_anggota);
    //     // console.log(anggota.data);
    //     this.anggota = anggota.data.anggota;
    ...
    backValue(){
      ...
      this.errors = {};
      // this.id_anggota = this.$route.params.id;
      const anggota = this.anggotas.find(k => k.id == this.id_anggota);
        if (anggota) {
          this.anggota = anggota;
          // console.log(anggota);
        }
