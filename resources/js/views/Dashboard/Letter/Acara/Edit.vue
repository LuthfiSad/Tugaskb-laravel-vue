        ...
        <div class="card-body">
          ...
          >
            ...
            <div class="form-group">
              ...
              />
              <img
                :src="assetUrl + 'storage/' + acara.image"
                width="300"
                style="max-width: 100%"
                class="mb-2"
                v-else-if="acara.image"
                alt=""
              />
              <button
  ...
  watch: {
    ...
    acaras: {
      ...
      async handler(items) {
        ...
        if (items.length > 0) {
          this.id_acara = this.$route.params.id;
          const acara = items.find((k) => k.id == this.id_acara);
          if (acara) {
            if (acara.deskripsi === null) {
              acara.deskripsi = "";
            }
            this.selectedDate = new Date(acara.jadwal);
            this.acara = acara;
            // document.querySelector(".ql-editor").innerHTML = (this.acara.deskripsi ? this.acara.deskripsi : '');
          } else {
            await Swal.fire({
  ...
  methods: {
    async formOnSubmit() {
      ...
      try {
        ...
        if (this.acara.image && this.imgPreview) {
          formData.append("image", this.acara.image);
        }
        // console.log(formData.get());
        const user = await axios.post("acara/" + this.id_acara, formData);
        this.acara = {};
        this.acara.id_kategori = "";
        this.errors = {};
        this.$emit("getAcara");
        await Swal.fire({
    ...
    backValue() {
      ...
      const anggota = this.acaras.find((k) => k.id == this.id_acara);
      if (anggota) {
        $('#image').val('');
    ...
    async resetButton() {
      ...
      if (!this.default && this.acara.image) {
