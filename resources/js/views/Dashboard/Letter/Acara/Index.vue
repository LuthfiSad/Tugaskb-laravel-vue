  ...
  <div class="row">
    <div class="col-12 mb-lg-0 mb-4">
      ...
      <div class="card z-index-2">
        ...
        >
          <h3 class="text-capitalize m-0">Data Acara</h3>
          <router-link
            to="acara/create"
            class="btn bg-gradient-primary px-1 mt-2 mb-0"
            >Tambah Acara</router-link
          >
        </div>
        <div class="card-body pt-0">
          <div class="table-responsive p-0">
            <DataTable :key="acaras.length" class="table align-items-center mb-0">
              <thead>
                <tr>
                  <td
              ...
              <tbody>
                <tr v-for="(acara, index) in acaras" :key="acara.id">
                  ...
                  <td>
                    <h6 class="mb-0 text-sm">{{ acara.title }}</h6>
                  </td>
                  <td>
                    <p class="text-xs font-weight-bold mb-0" :style="isJadwalBeforeToday(acara.jadwal)">
                      {{ getTanggal(acara.jadwal) }}
                    </p>
                  </td>
                  <td>
                    <p class="text-xs font-weight-bold mb-0" :style="isJadwalBeforeToday(acara.jadwal)">
                      {{ getWaktu(acara.jadwal) }}
                    </p>
                  </td>
                  <td>
                  ...
                  <td class="align-middle text-center" id="aksi">
                    <router-link
                      :to="'/dashboard/acara/detail/' + acara.id"
                      class="badge badge-sm bg-gradient-success font-weight-bold text-xs"
                    >
                      Detail
                    </router-link>
                    <router-link
                      :to="'/dashboard/acara/edit/' + acara.id"
                    ...
                    <button
                      v-on:click="deletedAcara(acara.id, acara.title)"
    ...
    async deletedAcara(id, title) {
      ...
      try {
        ...
        if (result.isConfirmed) {
          const response = await axios.delete("acara/" + id);
          this.$emit("getAcara");
          await Swal.fire({
